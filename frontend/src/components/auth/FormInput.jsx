const FormInput = ({ type, name, value, onChange, placeholder, error, isLoading, className, ...props }) => {
        ...
        <div className="space-y-1">
            <div className="relative">
                <input
                    ...
                    {...props}
                />
                {type === "password" && props.showPasswordToggle === true && (
                    <button
                        type="button"
                        onClick={props.onTogglePassword}
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700"
                    >
                        <i className={`ri-${props.showPassword ? 'eye-line' : 'eye-off-line'}`} />
                    </button>
                )}
            </div>
